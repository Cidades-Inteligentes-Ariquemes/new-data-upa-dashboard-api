...
impl DataProcessingForGraphPlotting {
    ...
    pub fn columns_to_plot_graphs() -> HashMap<String, Value> {
        ...
        columns.insert(
            ...
            json!(["ifrocompetencia"]),
        );
        columns.insert(
            "number_of_appointments_per_flow".to_string(),
            json!(["ifrocompetencia", "ifrotabelanome"]),
        );
        columns.insert(
            "distribuition_of_patients_ages".to_string(),
            json!(["ifrocompetencia", "ifropacienteidade"]),
        );
        columns.insert(
            "number_of_calls_per_day_of_the_week".to_string(),
            json!(["ifrocompetencia", "ifrodiasemana"]),
        );
        columns.insert(
    ...
    pub async fn create_dict_to_distribuition_of_patients_ages_from_raw(
        ...
        println!(
            "Amostra de competências: {:?}",
            &competencias[0..5.min(competencias.len())]
        );
        ...
        println!(
            "Idades válidas: {}, Idades inválidas: {}",
    ...
    pub async fn create_dict_to_distribution_of_services_by_hour_group(
        ...
        for i in 0..df.height() {
            let especialidade = df
                .column("ifroprofissionalcbods")?
                .str()?
                .get(i)
                .unwrap_or("");
    ...
    pub async fn create_dict_to_number_of_visits_per_nurse(
        ...
        let non_nurse_names: Vec<String> = df_non_nurse
    ...
    pub async fn create_dict_to_number_of_visits_per_doctor(
        ...
        let unique_doctors = df_filtered
            .column("ifroprofissionalnome")?
            .unique()?
            .str()?
            .into_iter()
    ...
    pub async fn create_dict_to_heat_map_with_the_number_of_medical_appointments_by_neighborhood(
        ...
        for i in 0..df.height() {
            ...
            if let (Some(b), Some(lat_str), Some(long_str), Some(end)) =
                (bairro, latitude_opt, longitude_opt, endereco)
            {
                // Filtrar endereços "DO IPE"
                if end != "DO IPE" {
                    // Converter latitude e longitude para números
