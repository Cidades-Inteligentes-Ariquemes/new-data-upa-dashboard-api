...
fn split_date_and_time(lf: LazyFrame) -> LazyFrame {
    ...
    .with_column(col("IfroDataAtendimento_temp").alias("IfroDataAtendimento"))
...
pub fn normalize_text_to_lower_case_columns_lazy(
    lf: LazyFrame,
    column_names: &[&str],
) -> LazyFrame {
    let mut result = lf;
...
pub fn normalize_text_to_upper_case_columns_lazy(
    lf: LazyFrame,
    column_names: &[&str],
) -> LazyFrame {
    let mut result = lf;
...
pub fn is_not_specified(text: &str) -> bool {
    text.trim().to_lowercase() == "não especificado"
        ...
        || ["encerramento de chamado", "atendimento chamado"]
...
pub fn get_disease_symptoms() -> HashMap<&'static str, Vec<&'static str>> {
    ...
    dicionario_sintomas.insert(
        ...
        vec![
            ...
            "dor na garganta",
            "tosse seca",
            "sensação de prurito na garganta",
...
pub fn extract_keyword_hybrid(df: &DataFrame) -> PolarsResult<DataFrame> {
    ...
    if !df
        .schema()
        .iter()
        .any(|(name, _)| name == "IfroConsultaConduta")
    {
        return Err(PolarsError::ComputeError(
    ...
    if !condutas_kmeans.is_empty() {
        ...
        let kmeans = KMeans::params(n_clusters)
            ...
            .fit(&dataset)
            .map_err(|e| {
                PolarsError::ComputeError(format!("Falha ao executar K-means: {}", e).into())
            })?;
        ...
        for cluster_idx in 0..n_clusters {
            ...
            let mut term_scores: Vec<(&String, f64)> = terms
                .iter()
                .enumerate()
            ...
            let top_terms: Vec<&String> = term_scores
                .iter()
                .take(top_n)
                .map(|(term, _)| *term)
                .collect();
...
fn diagnosticar_doenca_com_pontuacao(sintomas: &str) -> (String, f64) {
    ...
    for (doenca, sintomas_doenca) in dicionario_sintomas.iter() {
        ...
        if !sintomas_doenca.is_empty() {
            ...
            let porcentagem_sintomas =
                (sintomas_encontrados as f64 / sintomas_doenca.len() as f64) * 100.0;
...
pub fn normalize_column_names_of_the_df_to_lower_case(
    mut df: DataFrame,
) -> PolarsResult<DataFrame> {
    ...
    for (column_name, values) in data {
        ...
        match first_value {
            ...
            Some(value) if value.is_boolean() => {
                let bool_values: Vec<Option<bool>> = values.iter().map(|v| v.as_bool()).collect();
