...
impl PgDataRepository {
    ...
    async fn table_exists(&self, table: &str) -> Result<bool, Box<dyn Error + Send + Sync>> {
    ...
    pub async fn fetch_table_columns(
        ...
        if columns.is_empty() {
            return Err(std::io::Error::new(
                ...
                format!("Table {} does not exist or has no columns.", table),
            )
            .into());
        }
...
impl DataRepository for PgDataRepository {
    async fn fetch_all_data(
        ...
        if rows.is_empty() {
            info!("No data found in table {}", table);
        ...
        info!("Fetched all data from table {}", table);
        Ok(result)
    }

    async fn check_ifrocompetencia_exists(
        &self,
        table: &str,
        competencia_values: &[String],
    ...
    async fn create_table_if_not_exists(
        ...
        if !table_exists {
            if table == "bpa" {
                return Err(std::io::Error::new(
                    std::io::ErrorKind::NotFound,
                    "Tabela bpa não existe. Execute as migrations antes do upload.",
                )
                .into());
            }
            ...
            info!("SQL para criar tabela: {}", sql_command);
            ...
            sqlx::query(&sql_command)
                ...
                .await
                .map_err(|e| {
                    error!("Erro ao criar tabela {}: {}", table, e);
                    e
                })?;

            info!("Tabela {} criada com sucesso!", table);
        } else {
            info!("Tabela {} já existe. Anexando dados.", table);
        }
    ...
    async fn insert_data(
        ...
        match self.create_table_if_not_exists(df, table).await {
            Ok(true) => {
                ...
                let total_batches = if total_rows == 0 {
                    0
                } else {
                    total_rows.div_ceil(batch_size)
                };
                ...
                for batch_start in (0..total_rows).step_by(batch_size) {
                    ...
                    let filtered_columns: Vec<String> = df
                        .get_column_names()
                        .into_iter()
                    ...
                    if filtered_columns.is_empty() || df_batch.height() == 0 {
                        info!(
                            "[request_id={}] data_upload.insert_batch.skip table={} batch={}/{} reason=empty_batch_or_columns",
                            request_id, table, batch_number, total_batches
                        );
                        continue;
                    }

                    info!(
                        "[request_id={}] data_upload.insert_batch.start table={} batch={}/{} row_start={} row_end={} batch_rows={}",
                        request_id,
                        table,
                        batch_number,
                        total_batches,
                    ...
                    for row_idx in 0..df_batch.height() {
                        ...
                        for col_name in &filtered_columns {
                            // Iterate over references to owned Strings
                    ...
                    match sqlx::query(&sql_command).execute(&self.pool).await {
                        Ok(_) => {
                            info!(
                                "[request_id={}] data_upload.insert_batch.complete table={} batch={}/{} inserted_rows={} duration_ms={}",
                                request_id,
                                table,
                                batch_number,
                                total_batches,
                        ...
                        Err(e) => {
                            ...
                            error!(
                                "[request_id={}] Erro durante a inserção do lote {}/{} em {}: {}. SQL (truncada): {}",
                                request_id, batch_number, total_batches, table, e, truncated_query
                            );
                            return Err(e.into());
                        }
                    }
                ...
                info!(
                    "[request_id={}] Dados inseridos com sucesso na tabela {}. total_rows={}",
                    request_id, table, total_rows
                );
                Ok(true)
            }
            Ok(false) => {
                error!(
                    "[request_id={}] Falha ao criar tabela {}. Os dados não puderam ser inseridos.",
                    request_id, table
                );
                Ok(false)
            }
            Err(e) => {
                error!(
                    "[request_id={}] Erro ao criar tabela {}: {}",
                    request_id, table, e
                );
                Err(e)
            }
    ...
    async fn fetch_columns_by_name(
        &self,
        table: &str,
        columns: &[String],
    ) -> Result<HashMap<String, Vec<Value>>, Box<dyn Error + Send + Sync>> {
        ...
        if rows.is_empty() {
            println!(
                "No data found in table {} for columns {}",
                table,
                columns.join(", ")
            );
            return Ok(HashMap::new());
    ...
    async fn insert_nested_json(
        ...
        println!(
            "Dados inseridos/atualizados com sucesso na tabela {} com id {}",
            table, record_id
        );
    ...
    async fn fetch_nested_json(
        &self,
        table: &str,
        identifier: &str,
        unidade_id: Option<i32>,
    ) -> Result<serde_json::Map<String, serde_json::Value>, Box<dyn Error + Send + Sync>> {
    ...
    async fn fetch_distinct_values(
        ...
        if rows.is_empty() {
            println!(
                "Nenhum valor distinto encontrado para {} em {}",
                column, table
            );
            return Ok(Vec::new());
        }
                ...
                let value: Value = match type_name.as_str() {
                    ...
                    _ => {
                        ...
                        if row.try_get_raw(i)?.is_null() {
                            ...
                            println!(
                                "Erro ao obter valor de tipo desconhecido para {}",
                                column_name
                            );
                            Value::Null
                        }
    ...
    async fn insert_nested_json_with_unit(
        ...
        if !table_exists {
            ...
            if !column_exists {
                ...
                sqlx::query(&alter_table_query).execute(&self.pool).await?;
            }
        }
    ...
    async fn check_unit_data_exists(
        &self,
        table: &str,
        identifier: &str,
        unidade_id: i32,
