...
use crate::domain::{
    ...
    models::user::{
        AddApplicationDto, AddVerificationCodeDto, ConfirmVerificationCodeDto,
        CreateFeedbackOsteoporosisDto, CreateFeedbackRespiratoryDiseasesDto,
        CreateFeedbackTuberculosisDto, CreateUserDto, FeedbackRespiratoryDiseasesResponse,
        UpdateEnabledUserDto, UpdatePasswordByAdminDto, UpdatePasswordByUserCommonDto,
        UpdatePasswordForgettingUserDto, UpdateUserDto, UpdateVerificationCodeDto, UserResponse,
    },
...
use crate::infrastructure::{
    ...
    repositories::{data_upa_repository::PgDataRepository, user_repository::PgUserRepository},
};
...
use crate::utils::{
    ...
    validators::{
        is_valid_email, validate_applications, validate_feedbacks, validate_profile,
        validate_respiratory_diseases,
    },
};

use crate::utils::feedbacks_processing::{
    build_final_response, process_osteoporosis_stats, process_respiratory_stats,
...
impl UserService {
    ...
    pub async fn create_user(&self, user: CreateUserDto) -> Result<HttpResponse, AppError> {
        ...
        for (field_name, is_empty) in validations {
            if is_empty {
                return Err(AppError::BadRequest(format!(
                    "Error adding user: {} cannot be empty",
                    field_name
                )));
            }
        }
        ...
        if !is_valid_email(&user.email) {
            return Err(AppError::BadRequest(format!(
                "Error adding user: '{}' is not a valid email",
                user.email
            )));
        }
        ...
        let available_health_units = match self
            .data_repo
        ...
        for unit_id in &user.allowed_health_units {
            if !available_health_units.contains(unit_id) {
                return Err(AppError::BadRequest(format!(
                    "Error adding user: health unit with id '{}' does not exist",
                    unit_id
                )));
            }
        }
    ...
    pub async fn update_user(
        ...
        if self.repo.find_by_id(id).await.unwrap().is_none() {
            return Err(AppError::BadRequest(format!(
                "Error updating user: user with id '{}' not found",
                id
            )));
        }
        ...
        for (field_name, is_none) in validations {
            if is_none {
                return Err(AppError::BadRequest(format!(
                    "Error updating user: {} cannot be empty",
                    field_name
                )));
            }
        }
    ...
    pub async fn update_password_by_admin(
        ...
        for (field_name, is_empty) in _validations {
            if is_empty {
                return Err(AppError::BadRequest(format!(
                    "Error updating password: {} cannot be empty",
                    field_name
                )));
            }
        }
    ...
    pub async fn update_password_by_user_common(
        &self,
        id: Uuid,
        data: UpdatePasswordByUserCommonDto,
    ) -> Result<HttpResponse, AppError> {
        // Validações de campos vazios
        let validations = [
        ...
        if user.is_none() {
            return Err(AppError::BadRequest(format!(
                "Error updating password: user with id '{}' not found",
                id
            )));
        }
        ...
        if !self
            .password_encryptor
            .verify_password(&user.as_ref().unwrap().password, &data.current_password)
            .map_err(|e| {
    ...
    pub async fn update_enabled(
        ...
        if self.repo.find_by_id(id).await.unwrap().is_none() {
            return Err(AppError::BadRequest(format!(
                "Error deleting user: user with id '{}' not found",
                id
            )));
        }
    ...
    pub async fn add_application(
        ...
        if user.is_none() {
            return Err(AppError::BadRequest(format!(
                "Error adding application: user with id '{}' not found",
                id
            )));
        }
        ...
        for app in applications.applications_name.iter() {
            if user.as_ref().unwrap().allowed_applications.contains(app) {
                return Err(AppError::BadRequest(format!(
                    "Error adding application: application '{}' already exists",
                    app
                )));
            }
        }
    ...
    pub async fn delete_application(
        ...
        if user.is_none() {
            return Err(AppError::BadRequest(format!(
                "Error deleting application: user with id '{}' not found",
                id
            )));
        }
        ...
        if user.unwrap().allowed_applications.len() == 1 {
            return Err(AppError::BadRequest(
                "Error deleting application: user must have at least one allowed application"
                    .to_string(),
            ));
        }
    ...
    pub async fn create_feedback_respiratory_diseases(
        ...
        for (field_name, is_empty) in validations {
            if is_empty {
                return Err(AppError::BadRequest(format!(
                    "Error creating feedback: {} cannot be empty",
                    field_name
                )));
            }
        }
        ...
        validate_respiratory_diseases(&[
            feedback.prediction_made.clone(),
            feedback.correct_prediction.clone(),
        ])?;

        match self
            .repo
            .create_feedback_respiratory_diseases(feedback)
            .await
        {
    ...
    pub async fn create_feedback_tuberculosis(
        ...
        match self
            .repo
            .create_feedback_tuberculosis(feedback_tuberculosis)
            .await
        {
            Ok(feedback_tuberculosis) => {
                Ok(ApiResponse::created(feedback_tuberculosis).into_response())
            }
            Err(e) => {
                error!("Error creating feedback: {:?}", e);
                Err(AppError::InternalServerError)
    ...
    pub async fn create_feedback_osteoporosis(
        ...
        validate_feedbacks(&feedback.feedback)?;
        // Validação de feedbacks permitidos
        validate_feedbacks_osteoporosis(&[
            feedback.prediction_made.clone(),
            feedback.correct_prediction.clone(),
        ])?;
    ...
    pub async fn get_feedbacks(&self) -> Result<HttpResponse, AppError> {
        ...
        if respiratory_feedbacks.is_empty()
            || tuberculosis_feedbacks.is_empty()
            || osteoporosis_feedbacks.is_empty()
        {
            return Ok(
                ApiResponse::<Vec<FeedbackRespiratoryDiseasesResponse>>::feedbacks_not_found()
                    .into_response(),
            );
        }
        ...
        let final_response =
            build_final_response(respiratory_stats, tuberculosis_stats, osteoporosis_stats);
    ...
    pub async fn send_verification_code(&self, email: String) -> Result<HttpResponse, AppError> {
        ...
        if let Err(e) = email_service
            .send_email(user.full_name.clone(), email, final_code.clone())
            .await
        {
    ...
    pub async fn resend_verification_code(
        &self,
        email: String,
        id_verification: IdVerificationDto,
    ) -> Result<HttpResponse, AppError> {
        // Validação de email
        if !is_valid_email(&email) {
            return Err(AppError::BadRequest(format!(
        ...
        let code_exists = match self
            .repo
            .verify_code_exist(id_verification.id_verification)
            .await
        {
            Ok(code) => code,
            Err(e) => {
        ...
        match self
            .repo
            .update_code_verification(updated_code, email, id_verification.id_verification)
            .await
        {
    ...
    pub async fn confirm_verification_code(
        &self,
        data: ConfirmVerificationCodeDto,
    ) -> Result<HttpResponse, AppError> {
        //Verifica se o código existe no banco
        let code_exists = match self.repo.verify_code_exist(data.id_verification).await {
            ...
            Err(e) => {
                error!("Database error: {:?}", e);
                return Err(AppError::BadRequest(
                    "Id verification not found".to_string(),
                ));
            }
        };
        ...
        if code_exists.verification_code != data.verification_code {
            return Err(AppError::BadRequest(
                "Verification code not matched".to_string(),
            ));
        }
        ...
        match self
            .repo
            .update_used_verification_code(data.id_verification)
            .await
        {
    ...
    pub async fn update_password_for_forgetting_user(
        ...
        match self
            .repo
            .update_password_for_forgetting_user(user_id, new_password_hash.clone())
            .await
        {
            Ok(updated_password_hash) => {
                Ok(ApiResponse::success(updated_password_hash).into_response())
            }
            Err(e) => {
                error!("Error updating verification code: {:?}", e);
                Err(AppError::InternalServerError)
    ...
    pub async fn add_health_unit(
        ...
        if self.repo.find_by_id(id).await.unwrap().is_none() {
            return Err(AppError::BadRequest(format!(
                "Error adding health units: user with id '{}' not found",
                id
            )));
        }
        ...
        for unit_id in health_units.health_units.iter() {
            if !available_health_units.contains(unit_id) {
                return Err(AppError::BadRequest(format!(
                    "Error adding health units: health unit with id '{}' does not exist",
                    unit_id
                )));
            }
            ...
            if user.allowed_health_units.contains(unit_id) {
                return Err(AppError::BadRequest(format!(
                    "Error adding health units: health unit with id '{}' already exists",
                    unit_id
                )));
            }
        }
    ...
    pub async fn delete_health_unit(
        ...
        let user = match self.repo.find_by_id(id).await {
            ...
            Ok(None) => {
                return Err(AppError::BadRequest(format!(
                    "Error deleting health unit: user with id '{}' not found",
                    id
                )))
            }
            Err(e) => {
        ...
        if !user.allowed_health_units.contains(&health_unit_id) {
            return Err(AppError::BadRequest(format!(
                "Error deleting health unit: user does not have access to health unit with id '{}'",
                health_unit_id
            )));
        }
