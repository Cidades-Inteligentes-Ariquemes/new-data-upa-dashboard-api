...
use new_data_upa_dashboard_api::{
    adapters::{
        password::encryptor::Argon2PasswordEncryptor, token::generate_token::JwtTokenGenerator,
    },
    application::{
        auth_pronto_service::AuthProntoService, auth_service::AuthService,
        data_upa_service::DataUpaService, information_service::InformationService,
        machine_information_service::MachineInformationService,
        prediction_service::PredictionService, update_graph_data_service::UpdateGraphDataService,
        user_service::UserService, visualization_data_service::VisualizationDataService,
    },
    infrastructure::{
        database::{init_database, run_migrations},
        repositories::{
            audit_repository::PgAuditRepository,
            auth_pronto_repository::SqlServerAuthProntoRepository,
            data_upa_repository::PgDataRepository, user_repository::PgUserRepository,
        },
    },
    middleware::{audit::AuditMiddleware, auth::AuthMiddleware, logging::LoggingMiddleware},
...
async fn main() -> std::io::Result<()> {
    let env = Env::default().filter_or("RUST_LOG", "debug,actix_web=debug");
    ...
    let data_upa_service = web::Data::new(DataUpaService::new(data_repository.clone()));

    let update_graph_data_service =
        web::Data::new(UpdateGraphDataService::new(data_repository.clone()));
    ...
    let information_service = web::Data::new(InformationService::new(audit_repository.clone()));
